...
Вариант: `lisp | acc | harv | hw | instr | binary | stream | mem | cstr | prob1`
...
* прямая абсолютная адресация, например,

        LD [1000]

* относительная адресация со смещением, например,

        LD [SP + 100]
    
* косвенная относительная адресация (разымёнывание указателя):
...
4. `STACK_POINTER`
    ...
    Косвенная адресация относительно стекового регистра `SP`. (По сути, это указатель, хранящийся как переменная на стеке).
